...
import { PROYECTO_1 } from '@/lib/proyectos'
...
function llave(slug: string) {
  return `examen-web:proyecto:${P.id}:${slug || 'invitado'}`
}
...
export default function PaginaProyecto() {
  ...
  useEffect(() => {
    ...
    try {
      const crudo = localStorage.getItem(llave(slugificar(guardadoNombre)))
      if (crudo) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      localStorage.setItem(llave(slug), JSON.stringify(g))
    } catch {
      /* ídem */
    }
  }, [cargado, codigo, hechas, teclas, pegados, slug])
